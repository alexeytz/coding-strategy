...
| Counting your own tooling | `ps -ef \| grep -c` reported three leftover servers where there were none — the pipeline matched itself | A count is not a membership test, and the rule is forgotten first in instrumentation ([phase 8b §20](phase-8a-unattended-runs.md)) |
